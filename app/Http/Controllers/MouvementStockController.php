 ...
 *                 type="integer",
 *                 example=3,
 *                 description="Identifiant du bureau effectuant la demande"
 *             ),
 *
 *             @OA\Property(
 ...
 *                 description="Email du personnel responsable de la demande"
 *             ),
 *
 *             @OA\Property(
 *                 property="articles",
...
*/
    ...
    public function storeSortieStockMultiple(Request $request)
    {
        ...
        if (!empty($articlesInsuffisants)) {
            ...
            LogJournalisation::create([
                'action'     => 'Création de demande de Sortie de Stock Multiple',
                'ip_address' => $request->ip(),
                'user_agent' => $request->header('User-Agent'),
                'user_id'    => $request->user()->id,
                'user_name'   => $request->user()->name,
...
//1
    ...
    public function validAndUploadSigne(Request $request)
    {
        ...
        try {
            ...
            LogJournalisation::create([
                'action'     => "Telechargement de la demande signée",
                'ip_address' => request()->ip(),
                'user_agent' => request()->header('User-Agent'),
                'user_id'    => $request->user()->id,
                'user_name'   => $request->user()->name,
            ...
            LogJournalisation::create([
                'action'     => "Echec de Telechargement de la demande signée (exception)",
                'ip_address' => request()->ip(),
                'user_agent' => request()->header('User-Agent'),
                'user_id'    => $request->user()->id,
                'user_name'   => $request->user()->name,
    ...
    public function viewFile(Request $request)
    {
        ...
        if (!Storage::disk('public')->exists($sanitizedPath)) {
            ...
            LogJournalisation::create([
                'action'     => "Visualiser fichier mouvement stock",
                'ip_address' => request()->ip(),
                'user_agent' => request()->header('User-Agent'),
                'user_id'    => $request->user()->id,
                'user_name'   => $request->user()->name,
